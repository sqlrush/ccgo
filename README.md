...
- M5 初始核心工具：`internal/tools/file`、`internal/tools/bash`、`internal/tools/powershell`、`internal/tools/todo` 和 `internal/tools/web` 已提供 `Read`/`Write`/`Edit`、Bash、PowerShell、Glob/Grep、TodoWrite、WebFetch/WebSearch 初版，覆盖读前写、mtime stale guard、唯一匹配/`replace_all`、读结果去重、跨 tool round 的 read-state 保留、shell/PowerShell 结构化结果和基础安全分类。
