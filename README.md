...
- M6 初始上下文层：`internal/memory`、`internal/compact` 和 session search/title 支撑已落地，覆盖 CLAUDE.md/memdir 扫描、memory manifest、team-memory secret guard、compact 阈值/提示/runner/边界计划、conversation auto-compact 接入、失败熔断、microcompact/cache 初版、session memory summary/recall 初版、session 列表和 transcript 搜索。
- M7 初始 TUI 层：`internal/tui` 已提供轻量 terminal frame renderer、PromptInput 状态机、history 导航、keybinding resolver/config、vim insert/normal 基础、REPL screen 模型、permission/task dialog builder、dialog kind/id routing、status/dialog/message components、viewport 和 selection 模型。
...
- `internal/session`: Claude transcript JSONL 路径、读写和消息落盘。
...
- `internal/tui`: terminal frame renderer、prompt input、keybinding/config、vim 基础、REPL screen、permission/task dialogs、viewport/selection 和基础组件。
