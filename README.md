...
- M6 初始上下文层：`internal/memory`、`internal/compact` 和 session search/title 支撑已落地，覆盖 CLAUDE.md/memdir 扫描、memory manifest、team-memory secret guard、compact 阈值/提示/边界计划、session 列表和 transcript 搜索。
- M7 初始 TUI 层：`internal/tui` 已提供轻量 terminal frame renderer、PromptInput 状态机、history 导航、status/dialog/message components、viewport 和 selection 模型。
...
- `internal/compact`: compact prompt、token warning、auto-compact threshold 和 compact boundary plan。
- `internal/tui`: terminal frame renderer、prompt input、viewport/selection 和基础组件。
