...
- M7 初始 TUI 层：`internal/tui` 已提供轻量 terminal frame renderer、PromptInput 状态机、history 导航、keybinding resolver/config、vim insert/normal 基础、REPL screen 模型、permission/task dialog builder、dialog kind/id routing/runtime、stale dialog race guard、cancel active、task lifecycle 初版、alternate screen lifecycle 初版、ANSI snapshot 基础、snapshot corpus write/compare、scripted interaction runner/assertions 初版、status/dialog/message components、viewport 和 selection 模型。
