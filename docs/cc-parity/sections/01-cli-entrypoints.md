...
| CLI-FLAG-42 | `--include-partial-messages` stream-json 输出中包含部分消息块 | AUTO | 前置：已认证；操作：`claude -p "hi" --output-format stream-json --include-partial-messages`；预期：输出流中含 partial 消息 | `src/main.tsx:976`（`--include-partial-messages`） | ⚠️ 解析已接线（F2 commit TBD）；partial 消息块输出需要流式输出层改造 |
