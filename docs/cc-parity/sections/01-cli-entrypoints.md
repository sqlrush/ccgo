...
| CLI-FLAG-38 | `--file <specs>` 启动时下载文件资源（格式：`file_id:path`） | AUTO | 前置：已认证 Files API；操作：`claude -p "read doc" --file file_abc:doc.txt`；预期：doc.txt 被下载 | `src/main.tsx:1006`（`--file`） | ⚠️ 解析已接线（F2 commit TBD）；Files API 下载逻辑需要 Anthropic Files API 基础设施 |
...
| CLI-FLAG-41 | `--include-hook-events` 在 stream-json 输出中包含 hook 生命周期事件 | AUTO | 前置：已认证，有 hooks 配置；操作：`claude -p "hi" --output-format stream-json --include-hook-events`；预期：输出流中有 hook 事件 | `src/main.tsx:976`（`--include-hook-events`） | ⚠️ 解析已接线（F2 commit TBD）；hook 事件注入到 stream-json 需要流式输出层改造 |
...
| CLI-FLAG-43 | `--replay-user-messages` 将用户消息从 stdin 回显到 stdout（stream-json 模式） | AUTO | 前置：已认证；操作：stream-json 输入 + `--replay-user-messages`；预期：用户消息被回显 | `src/main.tsx:988`（`--replay-user-messages`） | ⚠️ 解析已接线（F2 commit TBD）；用户消息回显需要流式输出层改造 |
...
| CLI-FLAG-47 | `--plugin-dir <path>` 从目录加载插件（可重复） | AUTO | 前置：已认证，有插件目录；操作：`claude -p "hi" --plugin-dir ./myplugin`；预期：插件被加载 | `src/main.tsx:1006`（`--plugin-dir`） | ⚠️ 解析已接线（F2 commit TBD）；插件从目录加载需要插件加载器适配 |
