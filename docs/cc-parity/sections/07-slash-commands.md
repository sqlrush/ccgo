...
| CMD-TERMSETUP-01 | `/terminal-setup` 配置终端字体和颜色 | MANUAL | 前置:REPL 中; 操作:`/terminal-setup`; 预期:打开终端配置向导，检测并提示字体/颜色配置 | `src/commands/terminal-setup/index.ts:5` / `commands.ts:312` | ⚠️ 已建未接（F4 commit 12cabdf）（registered + router handler；返回 Shift+Enter 配置说明文本；实际 OS 级别 keybinding install 需平台特定代码；文本说明已覆盖主要终端） |
| CMD-BRANCH-01 | `/branch` 打开 git 分支管理界面 | MANUAL | 前置:git 仓库中; 操作:`/branch`; 预期:打开分支列表和切换界面 | `src/commands/branch/index.ts:5` / `commands.ts:262` | ⚠️ 已建未接（F4 commit 12cabdf）（registered + router handler；返回信息性消息；sidechain/branch overlay infra 尚未实现） |
...
| CMD-COPY-01 | `/copy` 将最后一条 assistant 消息复制到剪贴板 | MANUAL | 前置:有 assistant 响应; 操作:`/copy`; 预期:最后一条响应内容写入系统剪贴板 | `src/commands/copy/index.ts:5` / `commands.ts:269` | ⚠️ 已建未接（F4 commit 12cabdf）（copyHandler 注册于 production router；尝试 pbcopy/xclip/xsel/wl-copy；clipboard tool 不可用时返回错误信息；纯文本内容提取正常） |
...
| CMD-FAST-01 | `/fast` 切换 fast 模式（使用 Haiku 快速模型） | AUTO | 前置:任意; 操作:`/fast`; 预期:模型切换为 Haiku，后续响应更快 | `src/commands/fast/index.ts:5` / `commands.ts:278` | ⚠️ 已建未接（F4 commit 12cabdf）（registered + router handler；返回信息性消息指向 /model；运行时 Haiku model-switch infra 尚未实现） |
...
| CMD-COLOR-01 | `/color <color\|default>` 设置本次会话的提示栏颜色 | MANUAL | 前置:REPL 中; 操作:`/color blue`; 预期:提示栏颜色即时更改 | `src/commands/color/index.ts:9` / `commands.ts:266` | ⚠️ 已建未接（F4 commit 12cabdf）（registered + router handler；TUI REPLScreen 无 color-scheme 字段；返回⚠️提示消息） |
...
小计: 73 项 — ✅ 39 / ⚠️ 10 / ❌ 0 / N/A 24（F4 commit 12cabdf 新增 11 ✅ / 6 ⚠️；PRCMTS 由 ❌ 翻为 N/A；CMD-RESUME-02/CMD-MEMORY-01/CMD-MODEL-01 经 W-REPL 接线 commit 4c3d7f8 由 ⚠️ 翻为 ✅；G5 commit 184798d：CMD-TASKS-01 由 ⚠️ 翻为 ✅；CMD-BRANCH-01 仍 ⚠️（sidechain/branch overlay infra 尚未实现）；⚠️ 命令均已注册+接线，受限于 TUI infra 缺失；渲染需人工核验）
