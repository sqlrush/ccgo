...
M8 补充：CLI `plugin list` 现在会对 installed plugin roots 做显式加载诊断，坏 manifest 不再被静默跳过；JSON 输出会给对应 entry 带 `errors`，文本输出会标记 `failed to load` 并打印错误，和官方 list 对 load errors 可见的行为对齐。
...
M8 补充：CLI `plugin help|-h|--help` 和 `plugin marketplace help|-h|--help` 现在输出稳定 usage；缺少子命令或未知子命令会返回 exit 2 并显示 usage，不再落入 `unsupported subcommand` 占位文案。
...
M5 补充：PowerShell command canonicalization 现在保留与 PowerShell alias 碰撞的 `.exe`/`.cmd`/`.bat`/`.com` 原生命令名，避免 `fc.exe`、`sort.exe`、`sc.exe` 等被误当成 `Format-Custom`、`Sort-Object` 或 `Set-Content` 分类。

M5 补充：PowerShell native read-only classifier 现在覆盖 Windows `fc.exe` 和 `comp.exe` 的双文件比较安全子集，要求刚好两个安全相对路径，并拒绝空/单路径、裸 `comp`、越界路径、pass-through 和动态 `/n` 参数。
...
M5 补充：PowerShell read-only classifier 新增 `Split-Path` 安全路径字符串子集，允许安全相对 `Path`/`LiteralPath` 和 leaf/parent/qualifier/isAbsolute 输出形态，同时拒绝 `-Resolve`、绝对路径、父目录和变量路径。

M5 补充：PowerShell read-only classifier 新增 `Join-Path` 安全路径字符串子集，允许安全相对 `Path`/`LiteralPath`/`ChildPath`/`AdditionalChildPath` 和 positional 形态，同时拒绝 `-Resolve`、绝对路径和变量路径。
...
M5 补充：PowerShell read-only classifier 收紧 `Get-ItemProperty`/`Get-ItemPropertyValue` 与 `Get-FileHash`，继续允许普通 literal name/algorithm 参数，但拒绝动态表达式参数和越界路径。
...
M5 补充：PowerShell read-only classifier 收紧事件日志/CIM 查询类 cmdlet，`Get-EventLog` 和 `Get-CimClass` 继续允许普通 literal 查询参数，但拒绝 `$env:`、hashtable 等动态表达式参数。
...
M5 补充：PowerShell read-only classifier 收紧 `Get-WinEvent`，普通查询参数拒绝 `$env:`/hashtable 等动态表达式，`-FilterXPath` 走单独 XPath literal guard，允许 `[]`/`@Name` 语法但拒绝 PowerShell 动态表达式形态。
...
M8 补充：`/mcp remove|rm|delete <server>` 现在会从用户 `settings.json` 删除对应 `mcpServers` 配置，并同步清理该 server 的 allowed/denied MCP policy 与 runner 内存状态；project/local/plugin 来源会明确拒绝在 user settings 路径删除，避免误删非用户级来源。

M8 补充：`/status` built-in local-jsx command 现在在 headless/local runner 中返回基础 status text，包含 session id、working directory、model、tool count 和 settings-derived MCP server list，写入 result/transcript 且不会请求模型；`/status show <section>` 可只读展示 session/model/auth/tools/MCP/plugins/telemetry/bridge/remote/daemon/lsp/native/integrations 等运行态 section 详情，`/status all|full|dump` 与 `/status show all` 可组合输出全部 section，未知 section 返回 usage 而不是未实现占位。完整 TUI status panel、account/API connectivity、tool health 和插件状态仍未完成。
...
M8/M9 补充：`/mcp` built-in local-jsx command 现在有 headless/local 基础 text result：无参数或 `list` 会列出 settings-derived MCP server、transport 和 target，并对被 allow/deny policy 阻断的 server 标注 blocked reason，未配置时返回空状态；`show`/`info <server>` 会只读展示单个 server 的 status/policy/transport/source/target 和 env/header/OAuth 配置概要且不泄露 secret 值；`search/find <query>` 会按 server name、transport、source、target、env/header 名称、scope/plugin source 和 policy reason 做安全搜索；`refresh/reload` 会刷新 settings/plugin MCP server 状态，`restart/reconnect [server]` 会在 headless stateless lifecycle 下刷新配置并明确下一次 tool call 重新打开 server；`enable`/`disable` 会写回用户 `settings.json` 的 MCP allow/deny policy 并同步当前 runner 内存状态；`/mcp help` 和未知一级子命令会返回 usage，但仍不会连接/断开远端或请求模型。完整 MCP 管理 UI、实时启停 lifecycle 和健康检查仍未完成。
...
M8/M6 补充：`/config`、`/plugin`、`/memory` built-in local-jsx command 现在有 headless/local 基础 text result：`/config` 汇总工作目录、模型、settings 文件存在性和合并后 env/MCP/permissions/hooks/plugin 规模，`/config show <section>` 可只读展示 settings/model/output-style/auth/fast-mode/betas/env/permissions/MCP/hooks/plugins/marketplaces/sandbox 等单 section 详情且只暴露敏感配置键名不暴露值，`/config all|full|dump` 和 `/config show all` 可组合输出全部安全 section，未知一级子命令返回 usage，`/config search <query>` 可跨 settings/runtime/model/auth/env/permissions/MCP/hooks/plugins/marketplaces/sandbox 做安全搜索且不匹配 env/header/plugin option 等敏感值，`/plugin` 汇总 plugin settings/marketplace/registered plugin command 计数且未知一级子命令返回 help，`/memory` 汇总 session memory root、summary 数、relevant memory dir、memory 文件数和 recall/extraction 开关；`/memory show` 会只读列出 session summary 与 relevant memory markdown 文件预览，`/memory show <file>` 会在配置的 memory roots 内安全展示单个 markdown body 预览，`/memory search <query>` 只在配置的 memory roots 内搜索 markdown body/相对路径并返回命中预览，`/memory save|add|write <relative.md> <content>` 和 `/memory remove|rm|delete <relative.md>` 可在 relevant memory directory 内安全维护 markdown 记忆文件，并拒绝越界/非 markdown/symlink 目标，`/memory help` 与未知一级子命令返回 usage。这些命令不会请求模型，完整 local-jsx 面板、plugin marketplace TUI/install/update 和更完整 memory editor UI/局部编辑仍未完成。
...
M8/CLI 补充：`claude plugin marketplace refresh` 与 `reload` 现在作为 `update` 的别名进入同一 marketplace cache 刷新路径，不再被 CLI 分发层误报 unsupported；空 marketplace 状态与 `update` 保持一致。
