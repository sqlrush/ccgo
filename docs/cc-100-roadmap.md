...
M10 补充：daemon loopback endpoint 新增 `POST /stop`，CLI 新增 `--daemon-status`、`--daemon-stop` 和 `--daemon-state <path>`，可基于 session-scoped state 文件查询 runtime_state/endpoint/heartbeat 并优雅停止正在运行的 daemon；完整 detached start/restart、跨 session discovery 和远端托管仍未完成。
