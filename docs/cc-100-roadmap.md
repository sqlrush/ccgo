...
- `PowerShell` 初版已完成，支持 command/timeout/description/run_in_background 输入校验、`pwsh`/`powershell` 前台执行、后台启动、`PowerShellOutput` 输出读取、`KillPowerShell` 取消、stdout/stderr/exit code/timeout/cancel 结构化结果、缺失可执行文件结构化错误、动态 read-only/concurrency-safe/destructive 分类、文件读取类命令基础相对路径 guard 和默认工具注册。

仍需完成：

- `Read` 的完整 PDF parity、完整 notebook parity、完整 token-budget parity、full media parity、binary edge cases。
...
- `PowerShell` 完整 parser、完整权限/path validation、后台生命周期 edge cases、截断/落盘/session 记录和官方 golden；Notebook、MCP concrete tool semantics。
