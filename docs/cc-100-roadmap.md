...
当前状态：`cmd/claude-mcp` 仍是占位；已新增 `internal/mcp` 配置地基，覆盖 transport 归一化、stdio/url server signature、CCR proxy URL 解包、plugin MCP server 去重、allowed/denied MCP policy 的基础判定和过滤、MCP server config 环境变量展开、server scope/merge 基础、`.mcp.json` schema 解析/校验基础、项目目录链 `.mcp.json` 加载/合并、settings `mcpServers` scope 解析和 user/project/local 手工配置合并过滤、settings/.mcp.json/policy 到多 server toolset 的高层装配入口基础、conversation runner 对 configured MCP toolsets 的自动合并/执行基础、MCP tool result 归一化/大输出落盘基础、`mcp__server__tool` 名称归一化/解析 helper、MCP remote tool discovery/call 到 Go tool registry 的基础适配、MCP resource list/read helper tool 基础、MCP prompt list/get helper tool 基础、MCP JSON-RPC protocol client initialize/initialized lifecycle 和 session-expired 判定/reset/reinitialize/retry 基础、stdio newline JSON-RPC transport/process launch 基础、HTTP JSON-RPC transport 基础、HTTP event-stream response parsing、`mcp-session-id` header 复用和 DELETE close 基础、传统 SSE endpoint discovery + POST + async response stream 基础、WebSocket JSON-RPC transport 基础、stdio/HTTP/SSE/WS JSON-RPC notification 捕获/handler 基础、stdio/WS inbound server request handler 与 elicitation/create 默认 cancel/自定义 handler 基础、static authToken/OAuth beta transport header 基础、HTTP/SSE/WS dynamic auth header provider 基础、通用 OAuth refresh-token provider 基础、MCP OAuth server access-token provider 注入基础、stdio/HTTP/SSE/WS protocol client 到 server toolset 的装配基础，以及多 server toolset 聚合和统一 close 基础。CLI/TUI settings loader 到 runner 的完整接线、完整 SSE/WS lifecycle hardening、HTTP streaming lifecycle hardening、stdio lifecycle hardening、完整 OAuth 授权/secure storage/401 reactive token refresh、完整 channel notification/elicitation surface 和内置 MCP server 仍未完成。
