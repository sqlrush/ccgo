...
M6 补充：remote history pagination 现在也接受 `hasMoreResults`/`hasMoreItems`/`hasMorePages`、`isTruncated`/`truncated` 等继续分页标记，以及 `nextKey`/`lastEvaluatedKey`/`lastKey` cursor 别名，覆盖 keyset/token 风格分页响应。
