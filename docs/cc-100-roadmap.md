...
M7 补充：keybinding resolver/config 和脚本 named-key 输入已覆盖 `ctrl-h`/`ctrl-i`/`ctrl-j`/`ctrl-m`、`ctrl-[`、`ctrl-?` 及 `control-*` 终端别名；keybinding JSON loader 支持 wrapper object-map、`shortcuts`、object action 字段、`null`/`false` unbind。
...
本轮补充：keybinding config、keymap 解析和 interaction script named-key 输入接受 `ctrl-h`/`ctrl-i`/`ctrl-j`/`ctrl-m`、`ctrl-[`、`ctrl-?`、对应 `control-*` 以及 compact/camel 形式；keybinding JSON loader 支持 wrapper object-map、`shortcuts`、object action 字段、`null`/`false` unbind。
