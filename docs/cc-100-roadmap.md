...
| M10 Agents/tasks/worktree/remote | 已有 Task/TaskOutput/KillTask/SendMessage/TeamCreate/TeamDelete/TeamOutput/TeamSendMessage/TeamCoordinate/ResumeTask/Sleep/Brief/ScheduleCron/RemoteTrigger 入口、sidechain metadata/lifecycle、task progress event、显式与 settings 默认 owned worktree 创建/清理、sparse/symlink settings 应用、`run:true` subagent nested tool loop、agent permission mode/allowlist 过滤，以及 session-scoped team/schedule manifest、ScheduleCron manual trigger、team coordinator_task_id 元数据、TeamOutput coordinator status、TeamSendMessage target routing、coordinator briefing、structured handoff brief 和 remote trigger injection；完整远端连接服务、多 agent 调度和团队自动调度仍未完成 |
| 全量测试 | 当前 `go test ./...` 通过 |
...
M10 补充：新增 `ScheduleCron` 工具入口和 session-scoped `schedules.json` manifest，可 create/list/delete/trigger cron schedule metadata，校验 5-field cron 或常见 `@daily` 类表达式，并可绑定 team_id/target/message；`trigger` 会把保存的 schedule message 发送给绑定 team 的 running recipients。当前已有手动执行路径，完整后台 daemon 仍未完成。

M10 补充：新增 `RemoteTrigger` 工具入口，可把 source/event/message 作为远端触发事件注入到 running team recipients；默认优先发给 coordinator，消息正文保留远端来源和事件类型。完整 remote websocket/CCR 服务接入仍未完成。
...
当前状态：已有 Task/TaskOutput/KillTask/SendMessage/TeamCreate/TeamDelete/TeamOutput/TeamSendMessage/TeamCoordinate/ResumeTask/Sleep/Brief/ScheduleCron/RemoteTrigger 入口、sidechain metadata/lifecycle、team manifest、schedule manifest、ScheduleCron manual trigger、team coordinator_task_id 元数据、TeamOutput coordinator status、TeamSendMessage target routing、TeamCoordinate coordinator briefing、structured handoff brief、remote trigger injection、task progress event、显式与 settings 默认 owned worktree 创建/清理、sparse/symlink settings 应用、`run:true` subagent nested tool loop、agent permission mode 应用、agent tool allowlist registry/permission pattern 过滤，以及 completed/failed/cancelled 终态 owned worktree 自动清理；完整远端连接服务、多 agent 调度和团队自动调度仍未完成。
