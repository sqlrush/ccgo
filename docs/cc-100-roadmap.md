...
- `Glob`/`Grep` 纯 Go 初版已完成，支持 `**` 递归 glob、基础 ignored dirs、层级 `.gitignore`/`.ignore`、mtime/path 排序、Grep regex、glob/type 过滤、`files_with_matches`/`content`/`count` 输出模式、`context`/`before_context`/`after_context` 和 `-C`/`-B`/`-A` 上下文行、`offset`/`head_limit` 分页和大小写不敏感搜索。
