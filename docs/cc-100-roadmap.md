...
- `WebFetch` prompt-aware rendering、browser 渲染、preflight 和官方 golden；`WebSearch` 搜索后端/tool semantics。
...
1. 补强 M5 基础工具兼容：优先 `Bash` shell parser/sandbox/background 和 `WebSearch`，继续补 `Glob/Grep`、`TodoWrite`、`WebFetch` 的官方 golden 兼容。
