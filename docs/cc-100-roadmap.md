...
- `Glob`/`Grep` 纯 Go 初版已完成，支持 `**` 递归 glob、基础 ignored dirs、root-level `.gitignore`/`.ignore`、mtime/path 排序、Grep regex、glob 过滤和 `files_with_matches`/`content`/`count` 输出模式。
...
- `Glob/Grep` 完整 ripgrep parity、分页、层级 ignore 文件和更多输出参数。
