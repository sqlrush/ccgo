...
M6 补充：session-memory recall agent 现在也接受 `sessionPath`/`session_path`、`sessionSummaryPath` 和 `summaryPath`/`summary_path` selection aliases，模型/API 直接返回 summary 文件路径时可复用现有 path lookup 找回 session。
