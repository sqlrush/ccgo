...
M7 补充：keybinding resolver/config 和脚本 named-key 输入已覆盖 `ctrl-h`/`ctrl-i`/`ctrl-m` 及 `control-*` 终端别名。
...
本轮补充：keybinding config、keymap 解析和 interaction script named-key 输入接受 `ctrl-h`/`ctrl-i`/`ctrl-m`、`control-h`/`control-i`/`control-m` 以及 compact/camel 形式。
