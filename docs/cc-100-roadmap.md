...
M8 补充：新增基础 `ToolSearch` tool 并注册到默认内置工具集，executor 会把当前 tool registry 放入工具 metadata，`ToolSearch` 可按 name、alias、description、prompt、search hint 搜索当前可用工具定义，返回 BM25-ranked structured results 和 read-only/concurrency/destructive 元数据，并兼容 `query`/`q`/`search` 与 `topn`/`limit` 等输入别名。完整 deferred/lazy tool discovery 仍未完成。

M8/M2 补充：Anthropic request tool 转换现在会保留 contract 的 `strict` 与 `should_defer`，将 deferred 工具序列化为 API `defer_loading`，并用 `always_load` 覆盖 deferred hint；conversation runner 构造请求时会把 `Task` 等 deferred tool 的 strict/defer_loading 元数据带到最终请求。完整 deferred/lazy tool discovery 仍未完成。
