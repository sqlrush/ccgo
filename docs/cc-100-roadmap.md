...
当前状态：bootstrap/config/auth/model 基础已完成；CLI 已有 `--version`、scaffold settings 校验、基础 `--print` headless 单轮执行路径、基础 JSON result 输出、基础 stream-json event 输出和 raw streaming event 透传，但完整参数矩阵、交互 TUI 主循环、resume/continue、官方 SDK NDJSON/control protocol 和官方 stdout/stderr/exit-code parity 仍未完整兼容 CC。
