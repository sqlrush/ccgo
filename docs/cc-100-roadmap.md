...
M6 补充：memory 层新增官方 `relevant_memories` attachment 基础结构，支持 stable memory header、system-reminder 渲染和已 surfaced memory path/byte 扫描，为后续 selector/prefetch runtime 接入留好纯函数边界。
