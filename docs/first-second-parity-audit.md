...
- M6: full official microcompact/cached microcompact parity, richer session memory compaction and recall policy, complete memory recall agent strategy, complete sidechain/subagent transcript runtime, large transcript optimized loading, remote-history token refresh, and complete metadata entry coverage.
