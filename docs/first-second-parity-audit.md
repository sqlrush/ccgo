...
- `internal/compact`: effective context window calculations, auto-compact thresholds, token warning state, compact summary prompt construction, compact API runner, compact boundary/summary plan generation, microcompact/cache primitives, persistent cached microcompact storage, cache version/TTL/prune handling for disk and in-memory caches, atomic cache writes, corrupt-cache fail-open behavior, and filename/digest mismatch pruning.
