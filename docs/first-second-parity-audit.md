...
- `internal/compact`: microcompact disk cache loading now accepts Go default, camelCase, snake_case, and adjacent cache field aliases such as `content`/`text` summary, `cacheKey`/`key`/`hash` digest, cache-hit aliases, numeric strings for count fields, and RFC3339/Unix-seconds/Unix-millis time fields, so cached microcompact records from adjacent implementations remain usable.
