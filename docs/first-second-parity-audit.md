...
- `internal/compact`: microcompact disk cache loading now accepts provider-style response array wrappers such as `choices`, `outputs`, `candidates`, `generations`, and `results`, recovering summaries from nested `message.content`, `output.content`, and `content.parts[].text` while keeping outer cache metadata.
