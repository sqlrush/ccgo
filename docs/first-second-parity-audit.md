...
- `internal/compact`: microcompact disk cache loading now accepts Go default, camelCase, and snake_case field aliases, including numeric strings for count fields, so cached microcompact records from adjacent implementations remain usable.
...
- `internal/tui`: terminal CSI parsing now emits edit actions for REP repeat-preceding-character sequences such as `CSI b` and `CSI 4b`, and the visible-text/snapshot pipeline expands the previous repeatable grapheme by the requested count.
