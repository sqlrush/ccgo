...
- `internal/memory`: session-memory summaries can now be loaded, rolled up/pruned into an archive summary with prior rollup archives excluded from candidate compaction and merged across archive IDs, recalled by query with deterministic scoring and recency ordering, ranked through a model-assisted candidate/session-id selection path with fallback when selected IDs are invalid or excluded, alternate response-key parsing, fenced/prose JSON extraction, and scalar session-id parsing, and injected into resume context through the same optional recall agent path; deterministic and model-backed memory fact extraction can summarize user preferences, requests, decisions, and tool-use facts, including fenced/prose JSON, wrapped facts responses, and alternate fact field names.
