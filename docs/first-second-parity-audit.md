...
- `internal/session`: project session listing and pagination, lightweight transcript index/title/text-preview inference, streaming transcript search snippets for resume/search UI, sidechain transcript path/list/append helpers, sidechain runtime start/append/finish summary bridging, sidechain state/list/resume support, transcript tail/window loading for bounded-memory resume/UI paths, lightweight transcript metadata loading, and remote-history token refresh retry.
