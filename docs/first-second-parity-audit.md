...
- `internal/session`: project session listing and pagination, lightweight transcript index/title/text-preview inference, AI-title/last-prompt/task/agent/PR/worktree transcript metadata loading, streaming transcript search snippets for resume/search UI, sidechain transcript path/list/append helpers, sidechain runtime start/append/finish summary bridging, sidechain manager orchestration for spawn/append/finish/list/resume, sidechain state/list/resume support, transcript tail/window loading for bounded-memory resume/UI paths, lightweight transcript metadata loading, and remote-history token refresh retry.
...
- M6: full official microcompact/cached microcompact parity, official session memory compaction/recall policy beyond current rollup, complete memory recall agent strategy, complete sidechain/subagent orchestration beyond current runtime/state/resume bridge, large transcript indexed loading beyond current tail reader, remote-history token refresh, and remaining niche metadata entry semantics.
