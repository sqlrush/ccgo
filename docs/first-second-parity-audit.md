...
- `internal/compact`: microcompact disk cache loading now accepts Go default, camelCase, and snake_case field aliases, including numeric strings for count fields and RFC3339/Unix-seconds/Unix-millis time fields, so cached microcompact records from adjacent implementations remain usable.
