...
- `internal/compact`: compact runner summary responses now also unwrap provider-style `choices`, `outputs`, `candidates`, and `generations` payloads, recovering visible summary text from nested `message.content`, content-block arrays, and `content.parts[].text` before building the compact plan.
