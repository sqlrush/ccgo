...
type SidechainOptions struct {
	...
	WorktreePath        string
	WorktreeOwned       bool
	WorktreeSparsePaths []string
	WorktreeSymlinkDirs []string
	Description         string
	AgentPath           string
	AgentPrompt         string
	AgentModel          string
...
func (r SidechainRuntime) Start(options SidechainOptions) (SidechainRun, error) {
	...
	run := SidechainRun{
		...
		Metadata: SidechainMetadata{
			...
			WorktreeSparsePaths: append([]string(nil), options.WorktreeSparsePaths...),
			WorktreeSymlinkDirs: append([]string(nil), options.WorktreeSymlinkDirs...),
	...
	if err := AppendSidechainMessageInSubdir(r.SessionPath, r.SessionID, id, run.Subdir, TranscriptMessage{
		...
		Content: map[string]any{
			...
			"worktreeOwned":              options.WorktreeOwned,
			"worktreeSparsePaths":        append([]string(nil), options.WorktreeSparsePaths...),
			"worktreeSymlinkDirectories": append([]string(nil), options.WorktreeSymlinkDirs...),
			"description":                options.Description,
			"agentPath":                  options.AgentPath,
			"agentPrompt":                options.AgentPrompt,
			"agentModel":                 options.AgentModel,
