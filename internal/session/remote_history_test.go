...
func TestFetchRemoteHistoryAcceptsNumericBoolPageFields(t *testing.T) {
	var seen []url.Values
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		seen = append(seen, r.URL.Query())
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Query().Get("before_id") {
		case "":
			_, _ = w.Write([]byte(`{"data":[{"type":"status","session_id":"s","status":"latest"}],"has_next":1.0,"next_cursor":"evt_float_cursor"}`))
		case "evt_float_cursor":
			_, _ = w.Write([]byte(`{"data":[{"type":"status","session_id":"s","status":"older"}],"hasNext":0.0,"cursor":"ignored_when_complete"}`))
		default:
			t.Fatalf("unexpected before_id = %q", r.URL.Query().Get("before_id"))
		}
	...
	if len(seen) != 2 || seen[1].Get("before_id") != "evt_float_cursor" {
		t.Fatalf("queries = %#v", seen)
	}
}
...
func TestFetchRemoteHistoryAcceptsInlineFencedProviderPayloads(t *testing.T) {
	providerChoiceContent := func(t *testing.T, content string) []byte {
		t.Helper()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Query().Get("before_id") {
		case "":
			_, _ = w.Write(providerChoiceContent(t, "```json "+`{"events":[{"type":"status","event_id":"evt_inline","session_id":"s","status":"inline"}],"has_more":true,"first_id":"evt_inline"}`+" ```"))
		case "evt_inline":
			_, _ = w.Write(providerChoiceContent(t, "```json"+`{"events":[{"type":"status","event_id":"evt_glued","session_id":"s","status":"glued"}],"has_more":false}`+"```"))
		default:
			t.Fatalf("unexpected before_id = %q", r.URL.Query().Get("before_id"))
		}
	}))
	...
	if events.Events[0].ID != "evt_inline" || events.Events[0].Status != "inline" || events.Events[1].ID != "evt_glued" || events.Events[1].Status != "glued" {
		t.Fatalf("events = %#v", events.Events)
	}
	if len(seen) != 2 || seen[1].Get("before_id") != "evt_inline" {
		t.Fatalf("queries = %#v", seen)
	}
}
...
func TestFetchRemoteHistoryAcceptsLinkArrayCursors(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Query().Get("before_id") {
		case "":
			_, _ = w.Write([]byte(`{"data":[{"type":"status","session_id":"s","status":"latest"}],"links":[{"rel":"self","href":"/v1/sessions/s/events?cursor=self"},{"rel":"previous-page","href":"/v1/sessions/s/events?before-id=evt_array"}]}`))
		case "evt_array":
			_, _ = w.Write([]byte(`{"data":[{"type":"status","session_id":"s","status":"older"}],"links":[{"rel":["older_page"],"url":"/v1/sessions/s/events?last-evaluated-key=evt_array_old"}]}`))
		case "evt_array_old":
...
func TestFetchRemoteHistoryAcceptsLinkHeaderCursors(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Query().Get("before_id") {
		case "":
			w.Header().Set("Link", `</v1/sessions/s/events?cursor=self>; rel="self"; title="latest, self", </v1/sessions/s/events?before-id=evt_header&filter=alpha,beta>; rel="prev-page"`)
			_, _ = w.Write([]byte(`{"data":[{"type":"status","session_id":"s","status":"latest"}]}`))
		case "evt_header":
			w.Header().Set("Link", `</v1/sessions/s/events?older-cursor=evt_older&tags=one,two>; rel="older-page"`)
...
func TestFetchRemoteHistoryAcceptsSingularEventContainers(t *testing.T) {
	var seen []url.Values
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		seen = append(seen, r.URL.Query())
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Query().Get("before_id") {
		case "":
			_, _ = w.Write([]byte(`{"event":{"type":"status","event_id":"evt_event","session_id":"s","status":"event"},"hasMore":true,"firstId":"evt_event"}`))
		case "evt_event":
			_, _ = w.Write([]byte(`{"item":{"type":"status","eventId":"evt_item","sessionId":"s","status":"item"},"hasMore":true,"firstId":"evt_item"}`))
		case "evt_item":
			_, _ = w.Write([]byte(`{"record":{"type":"status","eventId":"evt_record","sessionID":"s","status":"record"},"hasMore":false}`))
		default:
			t.Fatalf("unexpected before_id = %q", r.URL.Query().Get("before_id"))
		}
	}))
	...
	for index, want := range []struct {
		...
		{id: "evt_event", status: "event"},
		{id: "evt_item", status: "item"},
		{id: "evt_record", status: "record"},
	} {
		if events.Events[index].ID != contracts.ID(want.id) || events.Events[index].Status != want.status {
			t.Fatalf("event %d = %#v, want %#v", index, events.Events[index], want)
		}
	}
	if len(seen) != 3 || seen[1].Get("before_id") != "evt_event" || seen[2].Get("before_id") != "evt_item" {
		t.Fatalf("queries = %#v", seen)
	}
}
...
func TestRemoteHistoryTranscriptMessagesAcceptsMetadataPayloadWrappers(t *testing.T) {
	var response sessionEventsResponse
	if err := json.Unmarshal([]byte(`{"events":[
		{"eventType":"user","eventId":"evt_u_meta","sessionID":"s_meta","createdAt":"2026-01-01T00:00:01Z","metadata":{"role":"user","text":"metadata hi"}},
		{"eventType":"assistant","eventId":"evt_a_props","sessionID":"s_meta","parentMessageID":"evt_u_meta","createdAt":"2026-01-01T00:00:02Z","properties":{"message":{"role":"assistant","message":"properties hello"}}}
	]}`), &response); err != nil {
		t.Fatal(err)
	}
	messages := RemoteHistoryTranscriptMessages(response.Events)
	if len(messages) != 2 || messages[0].UUID != "evt_u_meta" || messages[1].UUID != "evt_a_props" {
		t.Fatalf("messages = %#v", messages)
	}
	if messages[0].Message == nil || len(messages[0].Message.Content) != 1 || messages[0].Message.Content[0].Text != "metadata hi" {
		t.Fatalf("metadata payload = %#v", messages[0].Message)
	}
	if messages[1].Message == nil || len(messages[1].Message.Content) != 1 || messages[1].Message.Content[0].Text != "properties hello" {
		t.Fatalf("properties payload = %#v", messages[1].Message)
	}
	if messages[1].ParentUUID == nil || *messages[1].ParentUUID != "evt_u_meta" || messages[1].Message.ParentUUID == nil || *messages[1].Message.ParentUUID != "evt_u_meta" {
		t.Fatalf("parent linkage = %#v message=%#v", messages[1].ParentUUID, messages[1].Message)
	}
}
