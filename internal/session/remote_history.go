...
func (r *sessionEventsResponse) mergeScalarFields(raw map[string]json.RawMessage) error {
	...
	for _, spec := range []struct {
		...
		{name: "pagination_token", target: &r.Cursor},
		{name: "paginationToken", target: &r.CursorCamel},
		{name: "cursor_token", target: &r.Cursor},
		{name: "cursorToken", target: &r.CursorCamel},
		{name: "token", target: &r.Cursor},
...
func remoteHistoryEventMapReservedKey(key string) bool {
	...
	case "page", "pagination", "page_info", "pageInfo", "paging", "links", "_links", "meta", "metadata",
		...
		"pagination_token", "paginationToken", "cursor_token", "cursorToken", "token",
...
func remoteHistoryCursorField(raw map[string]json.RawMessage) string {
	return firstNonEmpty(
		...
		remoteHistoryStringField(raw, "pagination_token", "paginationToken"),
		remoteHistoryStringField(raw, "cursor_token", "cursorToken"),
		remoteHistoryStringField(raw, "token"),
...
func remoteHistoryProviderJSONPayload(text string) (string, bool) {
	...
	if lineEnd := strings.IndexAny(content, "\r\n"); lineEnd >= 0 {
		...
		if remoteHistoryProviderTextLooksJSON(candidate) {
