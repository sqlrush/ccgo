...
func TestPowerShellCommandClassification(t *testing.T) {
	readOnly := []string{
		...
		"Get-WinEvent -FilterXPath '*[System[Provider[@Name=\"PowerShell\"]]]' -MaxEvents 5",
	...
	notReadOnly := []string{
		...
		"Get-NetAdapter -Name $env:IFACE",
		"Get-NetIPAddress -InterfaceAlias $env:IFACE",
		"Get-NetIPConfiguration -InterfaceAlias @{N='Ethernet'}",
		"Get-NetRoute -DestinationPrefix $env:PREFIX",
		"Get-DnsClientCache -Name $env:HOST",
		...
		"Get-WinEvent -LogName $env:LOG",
		"Get-WinEvent -MaxEvents @{N=5}",
		"Get-WinEvent -FilterXPath $env:XPATH",
		"Get-WinEvent -FilterXPath @{N='x'}",
		"Get-WinEvent -FilterXPath (Get-Date)",
