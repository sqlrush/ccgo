...
const (
	...
	blockedSleepGuidance = "Run blocking commands in the background with run_in_background: true -- you'll get a completion notification when done. Use BashOutput to read output from a background command. If you genuinely need a delay, keep it under 2 seconds."
)
...
var powerShellReadOnlyCmdlets = map[string]powerShellReadOnlyConfig{
	...
	"join-path": {
		allowedFlags:               stringSet("path", "literalpath", "childpath", "additionalchildpath"),
		pathFlags:                  stringSet("path", "literalpath", "childpath", "additionalchildpath"),
		validatePositionalsAsPaths: true,
	},
	"get-filehash": {
