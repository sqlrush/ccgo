...
func TestParseModifiedNavigationKeySequences(t *testing.T) {
	...
	ss3Cases := []struct {
		...
		{seq: "\x8f1;2A", want: KeyUp},
		{seq: "\x8f1;3C", want: KeyAltRight},
		{seq: "\x8f1;5D", want: KeyCtrlLeft},
		{seq: "\x8f1;5F", want: KeyEnd},
	}
...
func TestParseFunctionKeySequences(t *testing.T) {
	cases := []struct {
		...
		{seq: "\x8f1;2P", want: KeyF1},
		{seq: "\x8f1;5Q", want: KeyF2},
...
func TestKeymapFromSpecsOverridesAndRemovesBindings(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "workbench.action.terminal.clear", want: ActionRedraw},
		...
		{name: "cursorWordLeft", want: ActionMoveWordLeft},
		{name: "editor.action.cursorWordStartLeft", want: ActionMoveWordLeft},
		{name: "wordForward", want: ActionMoveWordRight},
		{name: "nextWord", want: ActionMoveWordRight},
		{name: "cursorWordRight", want: ActionMoveWordRight},
		{name: "editor.action.cursorWordEndRight", want: ActionMoveWordRight},
		{name: "lineStart", want: ActionMoveStart},
		{name: "moveToBeginningOfLine", want: ActionMoveStart},
		{name: "cursorLineStart", want: ActionMoveStart},
		{name: "lineEnd", want: ActionMoveEnd},
		{name: "moveToEndOfLine", want: ActionMoveEnd},
		{name: "cursorLineEnd", want: ActionMoveEnd},
		...
		{name: "editor.action.deleteLeft", want: ActionDeleteBackward},
		...
		{name: "editor.action.deleteRight", want: ActionDeleteForward},
		{name: "deleteToBeginning", want: ActionDeleteToStart},
		{name: "deleteToStartOfLine", want: ActionDeleteToStart},
		{name: "deleteAllLeft", want: ActionDeleteToStart},
		{name: "killLine", want: ActionDeleteToEnd},
		{name: "deleteToEndOfLine", want: ActionDeleteToEnd},
		{name: "deleteAllRight", want: ActionDeleteToEnd},
		{name: "backwardKillWord", want: ActionDeleteWordBack},
		{name: "editor.action.deleteWordStartLeft", want: ActionDeleteWordBack},
		{name: "killWord", want: ActionDeleteWordFwd},
		{name: "editor.action.deleteWordEndRight", want: ActionDeleteWordFwd},
...
func TestREPLScreenVimVisualCharOperators(t *testing.T) {
	...
	typePromptText(&screen, "alpha beta gamma")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"0", "w", "v", "e", "x"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimNormal || screen.Prompt.Text != "alpha  gamma" || screen.Prompt.Cursor != len([]rune("alpha ")) || screen.VimRegister != "beta" || screen.VimRegisterLinewise {
	...
	typePromptText(&screen, "alpha beta gamma")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"0", "w", "v", "e", "s"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimInsert || screen.Prompt.Text != "alpha  gamma" || screen.Prompt.Cursor != len([]rune("alpha ")) || screen.VimRegister != "beta" || screen.VimRegisterLinewise {
	...
	typePromptText(&screen, "ALPHA beta gamma")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"0", "v", "e", "u"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimNormal || screen.Prompt.Text != "alpha beta gamma" || screen.Prompt.Cursor != 0 {
		t.Fatalf("after visual lowercase screen = %#v", screen)
	}
...
func TestREPLScreenVimVisualLineOperators(t *testing.T) {
	...
	typePromptText(&screen, "one\ntwo\nthree")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"G", "V", "k", "x"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimNormal || screen.Prompt.Text != "one" || screen.Prompt.Cursor != len([]rune("one\n"))-1 || screen.VimRegister != "two\nthree\n" || !screen.VimRegisterLinewise {
		t.Fatalf("after visual line x screen = %#v", screen)
	}
	...
	typePromptText(&screen, "one\ntwo\nthree")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"G", "V", "k", "s"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimInsert || screen.Prompt.Text != "one\n" || screen.Prompt.Cursor != len([]rune("one\n")) || screen.VimRegister != "two\nthree\n" || !screen.VimRegisterLinewise {
	...
	typePromptText(&screen, "one\ntwo\nthree")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"g", "g", "V", "j", "U"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimNormal || screen.Prompt.Text != "ONE\nTWO\nthree" || screen.Prompt.Cursor != 0 {
	...
	typePromptText(&screen, "one\ntwo\nthree\nfour")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"g", "g", "j", "V", "j", "o", "k", "y"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimNormal || screen.VimRegister != "one\ntwo\nthree\n" || !screen.VimRegisterLinewise || screen.Prompt.Text != "one\ntwo\nthree\nfour" {
...
func TestREPLScreenVimEditCommands(t *testing.T) {
	...
	screen = NewREPLScreen(40, 8, nil)
	screen.SetVimEnabled(true)
	typePromptText(&screen, "abcdef")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"0", "R", "X", "Y", "\x1b", "."} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimNormal || screen.Prompt.Text != "XYXYef" || screen.Prompt.Cursor != len([]rune("XYXY")) {
		t.Fatalf("after R dot-repeat screen = %#v", screen)
	}

	screen = NewREPLScreen(40, 8, nil)
	screen.SetVimEnabled(true)
	typePromptText(&screen, "ab")
	screen.ApplyKey(ParseKey("\x1b"))
	for _, seq := range []string{"0", "R", "X", "Y", "Z", "\x1b"} {
		screen.ApplyKey(ParseKey(seq))
	}
	if screen.VimMode != VimNormal || screen.Prompt.Text != "XYZ" || screen.Prompt.Cursor != len([]rune("XYZ")) {
		t.Fatalf("after R append screen = %#v", screen)
	}
...
func TestParseCSISequenceActions(t *testing.T) {
	...
	reportCases := []struct {
		...
		{seq: CSISequence(3, 120, 240, "t"), want: CSIReportAction{Type: CSIReportActionWindow, Code: 3, Params: []int{3, 120, 240}}},
		{seq: CSISequence(4, 600, 800, "t"), want: CSIReportAction{Type: CSIReportActionWindow, Code: 4, Params: []int{4, 600, 800}, Height: 600, Width: 800}},
		{seq: CSISequence(8, 24, 80, "t"), want: CSIReportAction{Type: CSIReportActionWindow, Code: 8, Params: []int{8, 24, 80}, Rows: 24, Columns: 80}},
		...
		{seq: CSISequence(12, 34, "R"), want: CSIReportAction{Type: CSIReportActionCursorPosition, Params: []int{12, 34}, Row: 12, Column: 34}},
		{seq: CSISequence("0;0R"), want: CSIReportAction{Type: CSIReportActionCursorPosition, Params: []int{0, 0}, Row: 1, Column: 1}},
		{seq: CSISequence("?7;8R"), want: CSIReportAction{Type: CSIReportActionCursorPosition, Params: []int{7, 8}, PrivateMode: '?', Row: 7, Column: 8}},
		{seq: CSISequence("?7;8;2R"), want: CSIReportAction{Type: CSIReportActionCursorPosition, Params: []int{7, 8, 2}, PrivateMode: '?', Row: 7, Column: 8, Page: 2}},
...
func TestParseOSCContent(t *testing.T) {
	...
	foreground := ParseOSCContent("10;rgb:f/0/8")
	if foreground.Type != OSCActionColor || foreground.Color.Target != "foreground" || !foreground.Color.Valid || foreground.Color.Color == nil || *foreground.Color.Color != (RGBColor{R: 255, G: 0, B: 136}) {
		t.Fatalf("foreground color = %#v", foreground)
	}
	backgroundQuery := ParseOSCContent("11;?")
...
func TestRunInteractionScriptAcceptsNestedClipboardImageFiles(t *testing.T) {
	steps, err := ParseInteractionScript([]byte(`[
		{
			"action": "paste",
			"payload": {
			...
			"expectPrompt": {
				"text": "[Image #1]",
				"expandedText": "[Image #1]",
				"pastedContentCount": 1,
				"pastedContents": {"id": 1, "type": "image", "mediaType": "image/png", "filename": "nested.png", "content": "CCCC", "sourcePath": "/tmp/nested.png"}
			}
		}
	]`))
	if err != nil {
...
func TestRunInteractionScriptAcceptsKeyEventObjects(t *testing.T) {
	steps, err := ParseInteractionScript([]byte(`[
...
func TestRunInteractionScriptAcceptsDOMResizePayloads(t *testing.T) {
	steps, err := ParseInteractionScript([]byte(`[
		{
			"type": "resize",
			"innerWidth": "62",
			"innerHeight": "13",
			"expectScreen": {"columns": 62, "rows": 13}
		},
		{
			"action": "resize",
			"payload": {
			...
			"expectScreen": {"columns": 64, "rows": 14}
		},
		{
			"operation": "screenSize",
			"payload": {
				"target": {
					"clientWidth": "66",
					"clientHeight": "15"
...
func TestRunInteractionScriptAcceptsWrappedKeyAliasFields(t *testing.T) {
	steps, err := ParseInteractionScript([]byte(`[
		{
			"action": "typeText",
			"value": "go",
...
func TestRunInteractionScriptAcceptsSingleExpectationArrayAliasFields(t *testing.T) {
	steps, err := ParseInteractionScript([]byte(`[
		{"input": "go", "expectPrompt": {"text": "go"}},
		{
			"key": "enter",
...
func TestRunInteractionScriptAcceptsResourceExpectationWrapperArrays(t *testing.T) {
	steps, err := ParseInteractionScript([]byte(`{
		"id": "resource-script",
		"type": "interaction-script",
		"attributes": {
			"steps": [
				{
					"action": "typeText",
					"value": "go",
					"assertions": [
						{"type": "prompt", "attributes": {"text": "go"}},
						{"target": "noEvent", "properties": "true"}
					]
				},
				{
					"type": "press",
					"value": "enter",
					"checks": [
						{"name": "event", "resource": {"type": "prompt_submitted", "value": "go"}},
						{"target": "eventCount", "attributes": 1},
						{"target": "prompt", "properties": {"empty": "yes"}}
					]
				}
			]
		}
